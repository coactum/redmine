  ...
  module Export
    module PDF
      module IssuesPdfHelper
        ...
        def issues_to_pdf(issues, project, query, jounalsMap)
          ...
          if table_width > 0 && query.has_column?(:history)
          ...
          issue_list(issues) do |issue, level|
            ...
            #if query.has_column?(:history) && jounalsMap[issue.id].present?
            #  pdf.set_x(10)
            ...
            if jounalsMap[issue.id].present?
              pdf.SetFontStyle('B',9)
              pdf.RDMCell(190,5, l(:label_history))
              pdf.ln
              jounalsMap[issue.id].each do |journal|
                pdf.SetFontStyle('B',8)
                title = "##{journal.indice} - #{format_time(journal.created_on)} - #{journal.user}"
                title << " (#{l(:field_private_notes)})" if journal.private_notes?
                pdf.RDMCell(190,5, title)
